...
- 📊 **实时使用监控** - 实时追踪 Claude Code 和 Codex 的 Token 使用量和请求次数
- 🎯 **多时间窗口统计** - 支持 5 小时、24 小时、当天、7 天、月度等多维度使用统计
- 🌐 **代理模式** - 可选本地代理，实现生成速率、TTFT、状态码等实时精准追踪
...
- 📋 **Claude Pro 支持** - 支持 Claude Pro 订阅的用量查询与监控
...
- 已安装 [Claude Code](https://claude.ai/code) 或 [Codex CLI](https://github.com/openai/codex)
...
| 模式         | 说明                                                                         | 功能差异                                                                                       |
| ------------ | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| **本地文件** | 默认模式。直接扫描并解析 Claude Code / Codex 会话 JSONL 文件（无需外部 CLI） | 支持配额窗口、Token/请求统计、模型分布、费用估算、会话、项目汇总和工具筛选                     |
| **本地代理** | 通过本地 Anthropic 兼容代理采集请求数据                                      | 额外支持生成速率、TTFT、状态码、请求耗时、API 来源筛选和代理侧请求记录等实时性能数据           |

> **提示**：
>
> - 本地文件模式是默认模式，无需安装任何外部工具，即可覆盖 Claude Code 和 Codex 的历史 Token、请求、会话、项目和费用统计。
...
│       ├── models/         # 数据模型
│       ├── proxy/          # 代理服务器实现
│       ├── session/        # 本地 JSONL 会话元信息扫描（Claude Code & Codex）
│       ├── local_usage/    # 本地文件用量数据库与 WebDAV 同步导出
│       ├── unified_usage/  # 本地文件 + 代理统一聚合层
│       ├── subscription/   # ChatGPT / Claude 订阅配额查询
...
本地文件用量数据库存储于 `~/.usagemeter/local_usage.db`（与代理数据库分离）；代理来源句柄保存于 `~/.usagemeter/proxy_source_handles.json`。

## 技术栈

- **前端**: Vue 3 + TypeScript + Vite + Tailwind CSS + Pinia + ECharts / vue-echarts
- **后端**: Tauri 2.x (Rust)，支持托盘图标、开机自启动、本地代理和原生窗口控制
- **数据**: 本地 Claude Code 与 Codex 会话 JSONL 解析、SQLite (`rusqlite`) 持久化、可选代理采集，以及统一合并统计事实层
