...
<template>
  <div class="space-y-2">
    <h3 class="px-1 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">
      {{ t(store.settings.locale, 'settings.dataManagement') }}
    </h3>
    <div class="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm divide-y divide-gray-50 dark:border-neutral-800 dark:bg-[#1C1C1E] dark:divide-neutral-800/50">
      <div
        class="cursor-pointer p-3 px-4 transition-colors hover:bg-gray-50 dark:hover:bg-neutral-800/50"
      ...
      >
        <div class="flex items-center justify-between">
          <div>
            ...
            <div class="mt-0.5 text-[10px] text-gray-400">{{ t(store.settings.locale, 'settings.modelPricingDesc') }}</div>
          </div>
          <svg class="h-4 w-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
          </svg>
      ...
      >
        <div class="flex items-center justify-between">
          <div>
            ...
            <div class="mt-0.5 text-[10px] text-gray-400">{{ t(store.settings.locale, 'settings.currencyDesc') }}</div>
          </div>
          <svg class="h-4 w-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
          </svg>
