...
<template>
  <div class="flex flex-col gap-3">
    ...
    <!-- 加载状态 -->
    <div v-if="customLoading" class="py-8 text-center text-sm text-[var(--theme-text-tertiary)]">
      {{ t(store.settings.locale, 'common.syncing') }}
    </div>

    <!-- 加载错误 -->
    <div v-else-if="loadError" class="theme-status-danger rounded-xl border p-3 text-xs">
    ...
    <template v-else>
      <!-- 匹配模式设置 -->
      <div class="theme-surface rounded-xl border p-3">
        <div class="flex items-center justify-between">
          <div>
            <div class="text-[13px] text-[var(--theme-text-primary)]">{{ t(store.settings.locale, 'settings.modelPricingMatchMode') }}</div>
            <div class="text-[10px] text-[var(--theme-text-tertiary)]">{{ t(store.settings.locale, 'settings.modelPricingMatchModeDesc') }}</div>
          </div>
          <div class="flex gap-1">
            <button
              ...
              :class="[
                ...
                localMatchMode === 'fuzzy' ? 'theme-button-accent' : 'theme-button-secondary'
              ]"
            >
            ...
            <button
              ...
              :class="[
                'px-2.5 py-1 rounded-md text-xs font-medium transition-all',
                localMatchMode === 'exact' ? 'theme-button-accent' : 'theme-button-secondary'
              ]"
            >
        ...
        <div class="mt-2 space-y-1 text-[10px] leading-snug text-[var(--theme-text-secondary)]">
        ...
        <!-- 最后同步时间 -->
        <div v-if="store.settings.modelPricing?.lastSyncTime" class="theme-divider mt-2 border-t pt-2 text-[10px] text-[var(--theme-text-tertiary)]">
          {{ t(store.settings.locale, 'settings.modelPricingLastSync') }}: {{ formatTime(store.settings.modelPricing.lastSyncTime) }}
        </div>
      </div>

      <!-- 同步错误 -->
      <div v-if="syncError" class="theme-status-danger rounded-xl border p-3 text-xs">
      ...
      <div class="flex gap-2">
        <button
          ...
          :class="[
            ...
            activeTab === 'custom'
              ? 'theme-button-accent'
              : 'theme-surface text-[var(--theme-text-secondary)] border'
            ]"
        >
      ...
      <template v-if="activeTab === 'custom'">
        ...
        <div class="relative">
          ...
          <input
            ...
            class="theme-input w-full rounded-xl py-2 pl-9 pr-10 text-sm"
          />
          <span v-if="customPricings.length > 0" class="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-[var(--theme-text-tertiary)]">{{ customPricings.length }}</span>
        </div>

        <!-- 添加按钮 -->
        <button
          @click="addCustom"
          class="theme-button-accent flex w-full items-center justify-center gap-1.5 rounded-xl py-2 text-[12px] font-medium transition-colors"
        >
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div v-else class="space-y-2 max-h-[250px] overflow-y-auto">
          <div v-if="customPricings.length === 0" class="py-8 text-center text-sm text-[var(--theme-text-tertiary)]">
          ...
          <div
            ...
            :key="pricing.modelId"
            class="theme-surface rounded-lg border px-2.5 py-2"
          >
            <!-- 第一行：模型名称 + 模型ID -->
            <div class="flex items-center gap-2 mb-1.5">
              ...
              <span
                ...
                class="flex-1 truncate text-[10px] text-[var(--theme-text-secondary)]"
              ...
              <div class="flex gap-0.5 shrink-0 ml-auto">
                <button @click="editPricing(pricing)" class="rounded p-1 transition-colors hover:bg-gray-100">
                ...
                <button @click="copyPricing(pricing)" class="rounded p-1 transition-colors hover:bg-gray-100" :title="t(store.settings.locale, 'settings.modelPricingCopy')">
                ...
                <button @click="openApplyModal(pricing)" class="rounded p-1 transition-colors hover:bg-gray-100" :title="t(store.settings.locale, 'settings.pricingApply')">
                ...
                <button @click="confirmDelete(pricing.modelId)" class="rounded p-1 transition-colors hover:bg-gray-100">
      ...
      <template v-else>
        ...
        <div class="relative">
          ...
          <input
            ...
            class="theme-input w-full rounded-xl py-2 pl-9 pr-10 text-sm"
          />
          <span v-if="syncedTotalCount > 0" class="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-[var(--theme-text-tertiary)]">{{ syncedTotalCount }}</span>
        </div>
        ...
        <div class="flex gap-2">
          <button
            @click="syncPricing"
            :disabled="syncing"
            class="theme-button-accent flex flex-1 items-center justify-center gap-1.5 rounded-xl py-2 text-[12px] font-medium transition-colors disabled:bg-blue-300"
          >
          ...
          <button
            ...
            :disabled="clearing"
            class="theme-button-secondary flex flex-1 items-center justify-center gap-1.5 rounded-xl py-2 text-[12px] font-medium transition-colors disabled:opacity-50"
          >
        ...
        <!-- 列表 -->
        <div v-if="syncedLoading" class="py-8 text-center text-sm text-[var(--theme-text-tertiary)]">
          {{ t(store.settings.locale, 'common.syncing') }}
        </div>
        <div v-else class="space-y-1 max-h-[300px] overflow-y-auto">
          <div v-if="syncedPricings.length === 0" class="py-8 text-center text-sm text-[var(--theme-text-tertiary)]">
          ...
          >
            ...
            <div class="flex items-center gap-2 mb-1.5">
              ...
              <span
                ...
                @mouseleave="hideTooltip"
              >{{ pricing.modelId }}</span>
              <button @click="openApplyModal(pricing)" class="ml-auto shrink-0 rounded p-1 transition-colors hover:bg-gray-100" :title="t(store.settings.locale, 'settings.pricingApply')">
                <svg class="w-3.5 h-3.5 text-gray-400 hover:text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <rect width="16" height="20" x="4" y="2" rx="2" stroke-width="2" /><line x1="8" x2="16" y1="6" y2="6" stroke-width="2" /><path d="M16 10h.01M12 10h.01M8 10h.01M16 14h.01M12 14h.01M8 14h.01M16 18h.01M12 18h.01M8 18h.01" stroke-width="2" stroke-linecap="round" />
                </svg>
              </button>
    ...
    <Teleport to="body">
      <div
        v-if="tooltipVisible"
        class="fixed z-[9999] max-w-[300px] break-all rounded-md px-2 py-1 text-[11px] pointer-events-none theme-modal-panel theme-text-primary"
        ...
        <div class="theme-modal-panel w-[280px] overflow-hidden rounded-2xl" @click.stop>
          <div class="p-4">
            <div class="text-center">
              ...
              <h3 class="mb-1 text-sm font-medium text-[var(--theme-text-primary)]">
          ...
          <div class="theme-divider flex border-t">
            ...
            <button
              @click="deletePricing"
              class="theme-divider flex-1 border-l py-2.5 text-xs font-medium text-[var(--theme-status-danger-fg)] transition-colors hover:bg-red-50"
            >
