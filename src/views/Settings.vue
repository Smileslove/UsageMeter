...
// ---- 本地缓存管理 ----
// 阶段 3 UI：暴露 purge_orphan_local_facts / rebuild_local_usage_cache 给用户。
// 后端在阶段 1 已实现软删除（用户删除 JSONL 不丢统计），但软删的行需要一个用户出口
// 才能避免数据库无界增长。
...
// 0 = 全部；其他值为天数。和后端 purge_orphan_local_facts(older_than_days) 对齐。
...
const loadLocalCacheStats = async () => {
  try {
    ...
    localCacheMessage.value = `${t(store.settings.locale, 'settings.localCacheLoadFailed')}: ${e}`
...
const confirmLocalCacheAction = async () => {
  ...
  try {
    ...
    localCacheMessage.value = `${t(store.settings.locale, 'settings.localCacheActionFailed')}: ${e}`
    localCacheMessageIsError.value = true
  } finally {
...
<template>
  <div class="relative">
          ...
          <div class="p-3 px-4">
            <div class="mb-3 flex items-start justify-between gap-3">
              <div class="min-w-0">
                ...
                <div class="text-[10px] text-gray-400 mt-0.5 leading-relaxed">{{ t(store.settings.locale, 'settings.localCacheDesc') }}</div>
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-2 mb-3">
              <div class="rounded-xl border border-gray-100 bg-white px-2.5 py-2 dark:border-neutral-800 dark:bg-neutral-950">
                ...
                <div class="text-[10px] text-gray-400 dark:text-gray-500 truncate" :title="t(store.settings.locale, 'settings.localCacheTotalFactsDesc')">
              ...
              <div class="rounded-xl border border-gray-100 bg-white px-2.5 py-2 dark:border-neutral-800 dark:bg-neutral-950">
                ...
                <div class="text-[10px] text-gray-400 dark:text-gray-500 truncate" :title="t(store.settings.locale, 'settings.localCacheOrphanFactsDesc')">
            ...
            <!-- 清理孤立记录 -->
            <div class="flex items-center justify-between gap-2 rounded-xl border border-gray-100 bg-white px-2.5 py-2 mb-2 dark:border-neutral-800 dark:bg-neutral-950">
              <div class="min-w-0">
            ...
            <div class="flex items-center justify-between gap-2 rounded-xl border border-gray-100 bg-white px-2.5 py-2 dark:border-neutral-800 dark:bg-neutral-950">
              <div class="min-w-0">
                ...
                <div class="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5 truncate" :title="t(store.settings.locale, 'settings.localCacheRebuildDesc')">
    ...
    <Teleport to="body">
      <div
        v-if="localCacheConfirmMode"
        class="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4"
        @click.self="closeLocalCacheConfirm"
      >
        <div class="w-[320px] overflow-hidden rounded-2xl border border-white/70 bg-white shadow-xl dark:border-neutral-800 dark:bg-[#1C1C1E]" @click.stop>
          <div class="p-4">
            <h3 class="text-sm font-semibold text-gray-900 dark:text-gray-100">
              {{ t(store.settings.locale, localCacheConfirmMode === 'purge-orphan' ? 'settings.localCachePurgeConfirmTitle' : 'settings.localCacheRebuildConfirmTitle') }}
            </h3>
            <p class="mt-2 text-xs leading-relaxed text-gray-500 dark:text-gray-400">
              {{ t(store.settings.locale, localCacheConfirmMode === 'purge-orphan' ? 'settings.localCachePurgeConfirmBody' : 'settings.localCacheRebuildConfirmBody') }}
            </p>
          </div>
          <div class="flex border-t border-gray-100 dark:border-neutral-800">
            <button
            ...
            <button
              class="flex-1 border-l border-gray-100 py-2.5 text-xs font-medium text-red-600 transition-colors hover:bg-red-50 disabled:opacity-50 dark:border-neutral-800 dark:text-red-400 dark:hover:bg-red-500/10"
              :disabled="localCacheBusy"
              @click="confirmLocalCacheAction"
            >
              {{ localCacheBusy ? t(store.settings.locale, 'common.syncing') : t(store.settings.locale, 'common.confirm') }}
            </button>
          </div>
        </div>
      </div>
